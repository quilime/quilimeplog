    ...
    extends HTTP_Request
    {
        ...
        */
        function sign($authHeader=false)
        {
            ...
            $parameter_adder = (isset($this->_requestHeaders['content-type']) && in_array($this->_requestHeaders['content-type'], array('application/x-www-form-urlencoded', 'multipart/form-data')))
            ...
            $parameters_to_normalize = array_merge($this->_url->querystring,
                                                   ($this->_postData ? $this->_postData : array()),
            ...
            $oauth_parameters['oauth_signature'] = HTTP_Request_OAuth::_md5($signed_string);

            if($authHeader) {
                // oauth_* params go into the Authorization request header
                $authorization_header = "OAuth realm=\"{$this->_realm}\"";
